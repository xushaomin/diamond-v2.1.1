...
public class PersistService {
    ...
    private static final class ConfigUserRowMapper implements ParameterizedRowMapper<ConfigUser> {
        public ConfigUser mapRow(ResultSet rs, int rowNum) throws SQLException {
        	ConfigUser info = new ConfigUser();
            info.setId(rs.getLong("id"));
            info.setUserIds(rs.getString("user_ids"));
            return info;
        }
    }
    ...
    public Page<ConfigInfo> findConfigInfoLike2(final int pageNo, final int pageSize, final String keyword, User user) {
        ...
        String sqlCountRows = "select count(id) from config_info where 1=1 ";
        String sqlFetchRows = "select id,data_id,group_id,content,md5 from config_info where 1=1 ";
        if (!StringUtils.isBlank(keyword)) {
        	sqlCountRows += "and (data_id like '%" + keyword + "%' or group_id like '%" + keyword + "%' ) ";
        	sqlFetchRows += "and (data_id like '%" + keyword + "%' or group_id like '%" + keyword + "%' ) ";
        }
        ...
        if(user.getIsadmin() == 0 && user.getRoles().indexOf(CasPropertiesConfig.getString("configRoleId")) == -1) {
        	Integer userId = user.getId();
        	
        	sqlCountRows += "and id in (select id from config_user where user_ids like '%" + userId + "%' ) ";
        	sqlFetchRows += "and id in (select id from config_user where user_ids like '%" + userId + "%' ) ";
        }
